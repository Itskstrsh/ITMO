...
module Make (Ord : Ordered.ORDERED) : S with type key = Ord.t = struct
  ...
  let rec push_left t st =
    match t with E -> st | T (_, l, _, _, _) as n -> push_left l (n :: st)
  ...
  let rec map g = function
    | E ->
        E
    | T (c, l, k, v, r) ->
        T (c, map g l, k, g v, map g r)

  let filter p t =
    fold_left (fun acc k v -> if p k v then add k v acc else acc) empty t
