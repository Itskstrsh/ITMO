...
let of_list xs = List.fold_left (fun acc (k, v) -> D.add k v acc) D.empty xs
...
let test_basic_ops () =
  ...
  Alcotest.(check (option string))
    "map keeps value" (Some "B") (D.find_opt 2 d2) ;
...
let gen_dict =
  ...
  |> set_shrink Shrink.list |> map of_list

(* 1) mem <-> find_opt *)
let prop_mem_find =
  QC.Test.make ~name:"mem <-> find_opt"
    QC.(pair small_int gen_dict)
    (fun (k, d) -> D.mem k d = Option.is_some (D.find_opt k d))
...
let prop_monoid_identity =
  QC.Test.make ~name:"monoid identity" gen_dict (fun d ->
      D.equal ( = ) (D.append combine_left d D.empty_monoid) d
      && D.equal ( = ) (D.append combine_left D.empty_monoid d) d )
