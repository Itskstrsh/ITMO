module Euler22 = struct
  ...
  let split_csv_names (s : string) : string list =
    s |> remove_all_quotes |> String.split_on_char ',' |> List.map String.trim
    |> List.filter (fun t -> t <> "")
  ...
  let name_value (name : string) : int =
    ...
    in
    let rec loop i acc =
      if i = String.length name then acc else loop (i + 1) (acc + score name.[i])
    in
    loop 0 0
  ...
  module Score = struct
    let by_name ((i, nm) : int * string) : int = i * name_value nm
  end
  ...
  let modular_score (names_sorted : string list) : int64 =
    names_sorted |> Index.attach |> List.map Score.by_name
